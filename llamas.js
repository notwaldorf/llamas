...
function startDoingThings()
{
	...
	var url = "http://query.yahooapis.com/v1/public/yql?q=select%20*%20from%20answers.search%20where%20query%3D%22llamas%22&format=json";

	$.ajax({
	  url: url,
	  success: doSomethingWithTheResults,
	  error: doSomethingWithNoResultsAtAll,
	  dataType: "json"
	});
}
function doSomethingWithNoResultsAtAll()
{
	hardcodeForTesting();
	displayRandomBit();	
}
function doSomethingWithTheResults(data)
{
	theGoodBits = parseTheGoodBits(data);
	displayRandomBit();
}

function parseTheGoodBits(data){
	var theseOnes = [];
	...
	{
		theseOnes.push({});
		theseOnes[i].subject = questions[i].Subject;
		theseOnes[i].content = questions[i].Content;
		theseOnes[i].answer = questions[i].ChosenAnswer;
	}
	return theseOnes;
}

function displayRandomBit()
{
...
function hardcodeForTesting()
{	// all the typos are from yahoo answers.
	theGoodBits = [
		{
