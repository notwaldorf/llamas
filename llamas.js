...
function startDoingThings()
{
	...
	var url="http://query.yahooapis.com/v1/public/yql?q=select%20*%20from%20answers.search(0%2C50)%20where%20query%3D%22llamas%22&format=json";
	$.ajax({
	  url: url,
	  success: function(data) { parseTheGoodBits(data); },
	  dataType: "json"
	});
}

function immediateViewing()
{
	loadHardcodedStuff();
	haveAFact();	
}
...
function parseTheGoodBits(data){
	...
	// first populate the questions we have
	for (var i in theGoodBits)
	{
		questionsWeHave[theGoodBits[i].id] = 1;
	}

	var questions = data.query.results.Question;
	for (i in questions)
	{
		// we only want questions we don't have
		if (!questionsWeHave[questions[i].id])
		{
			theGoodBits.push({});
